<template>
  <v-container class="d-flex flex-wrap">
    <v-card class="mx-auto ma-3" max-width="500" width="350" v-for="t of trainings" :key="t.trainings_id">
      <v-card-text>
        <div>
          <v-icon class="mr-3" color="orange darken-2">{{ icon }}</v-icon>
          <span class="text-h6 black--text">Training am: {{ t.datum.substring(0, 10) }}</span>
        </div>
        <div class="text--primary mt-5 ml-9">
          <v-icon small>mdi-clock-time-nine-outline</v-icon
          ><span class="ml-2">{{ t.beginn.substring(0, 5) }} - {{ t.ende.substring(0, 5) }}</span>
        </div>
        <div class="text--primary mt-1 ml-9">
          <span class="font-weight-bold"> Anmeldefrist: </span>
          <v-chip class="red white--text">{{ t.eintragefrist.substring(0, 10) }}</v-chip>
        </div>
        <div class="text--primary mt-1 ml-9">
